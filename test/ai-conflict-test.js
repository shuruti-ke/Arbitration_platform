...
(async () => {
  ...
  try {
    const result = await aiConflictScanner.scanForConflicts(testCaseData);
  ...
  const logId = await auditTrail.logEvent({
  ...
  console.error('AI Conflict, WORM Storage, and Audit Trail tests failed:', error);
  process.exitCode = 1;
});
